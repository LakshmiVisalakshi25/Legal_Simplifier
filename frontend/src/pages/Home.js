...
export default function Home() {
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {/* Navbar */}
      <div style={{ background: '#fff', padding: '16px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a1a2e' }}>⚖️ LegalSimplifier</div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={() => navigate('/auth')} style={{ background: 'transparent', border: '1px solid #667eea', color: '#667eea', padding: '8px 20px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>Login</button>
          <button onClick={() => navigate('/auth')} style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none', color: '#fff', padding: '8px 20px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>Get Started Free</button>
        </div>
      </div>

      {/* Hero Section */}
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', padding: '80px 40px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '48px', fontWeight: '800', color: '#fff', marginBottom: '20px', lineHeight: '1.2' }}>
          ...
          <span style={{ background: 'linear-gradient(135deg, #667eea, #a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>in 10 Seconds</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/auth')} style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none', color: '#fff', padding: '14px 36px', borderRadius: '10px', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>
            Analyze My Document →
          </button>
          <button onClick={() => navigate('/auth')} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.3)', color: '#fff', padding: '14px 36px', borderRadius: '10px', fontSize: '16px', cursor: 'pointer' }}>
            See How It Works
          </button>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '48px', marginTop: '60px', flexWrap: 'wrap' }}>
          {[['10 sec', 'Analysis time'], ['100%', 'Free to use'], ['50+', 'Document types'], ['Plain', 'Simple language']].map(([num, label]) => (
      ...
      <div style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', padding: '80px 40px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.8)', marginBottom: '36px' }}>Understand what you're agreeing to before it's too late</p>
        <button onClick={() => navigate('/auth')} style={{ background: '#fff', border: 'none', color: '#667eea', padding: '16px 48px', borderRadius: '10px', fontSize: '16px', fontWeight: '700', cursor: 'pointer' }}>
